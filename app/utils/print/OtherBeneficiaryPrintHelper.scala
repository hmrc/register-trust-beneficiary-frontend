...
class OtherBeneficiaryPrintHelper @Inject() (answerRowConverter: AnswerRowConverter) extends PrintHelper {
  ...
  override def answers(userAnswers: UserAnswers, name: String, index: Int, draftId: String)(implicit
    ...
    Seq(
      ...
      bound.yesNoQuestion(
        UKResidentYesNoPage(index),
        "otherBeneficiary.ukResidentYesNo",
        UKResidentYesNoController.onPageLoad(index, draftId).url
      ),
