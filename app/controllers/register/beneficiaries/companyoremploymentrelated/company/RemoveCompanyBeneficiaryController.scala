...
class RemoveCompanyBeneficiaryController @Inject() (
  ...
  override val messagesPrefix: String = "removeCompanyBeneficiaryYesNo"
  ...
  override def formRoute(draftId: String, index: Int): Call =
    routes.RemoveCompanyBeneficiaryController.onSubmit(index, draftId)

  override def removeQuery(index: Int): Settable[_] = RemoveCompanyBeneficiaryQuery(index)
