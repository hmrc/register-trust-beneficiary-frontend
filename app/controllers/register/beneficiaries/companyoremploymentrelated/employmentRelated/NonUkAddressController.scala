...
class NonUkAddressController @Inject() (
  ...
  def onPageLoad(index: Int, draftId: String): Action[AnyContent] =
    standardActionSets.identifiedUserWithData(draftId).andThen(nameAction(index)) { implicit request =>
      val preparedForm = request.userAnswers.get(LargeBeneficiaryAddressInternationalPage(index)) match {
        case None        => form
        ...
        .fold(
          ...
          value => {
            val result = for {
              updatedAnswers <-
                EitherT(
                  Future.successful(request.userAnswers.set(LargeBeneficiaryAddressInternationalPage(index), value))
                )
              _              <- sessionRepository.set(updatedAnswers)
            } yield Redirect(
              navigator.nextPage(LargeBeneficiaryAddressInternationalPage(index), draftId, updatedAnswers)
            )
