...
class CountryOfResidenceInTheUkYesNoController @Inject() (
  ...
  private val form: Form[Boolean] =
    formProvider.withPrefix("employmentRelatedBeneficiary.5mld.countryOfResidenceInTheUkYesNo")
  ...
  def onSubmit(index: Int, draftId: String): Action[AnyContent] = actions(draftId, index).async { implicit request =>
    form
      .bindFromRequest()
      .fold(
        formWithErrors => Future.successful(BadRequest(view(formWithErrors, draftId, index, request.beneficiaryName))),
        value => {
          val result = for {
            updatedAnswers <-
