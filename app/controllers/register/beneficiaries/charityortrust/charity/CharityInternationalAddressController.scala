...
class CharityInternationalAddressController @Inject()(
  ...
  def onPageLoad(index: Int, draftId: String): Action[AnyContent] =
    ...
    implicit request =>
      ...
      Ok(view(preparedForm, countryOptions.options(request.lang(messagesApi)), draftId, index, charityName))
  }
    ...
    implicit request =>
      ...
      form.bindFromRequest().fold(
        (formWithErrors: Form[_]) =>
          Future.successful(BadRequest(view(formWithErrors, countryOptions.options(request.lang(messagesApi)), draftId, index, charityName))),
