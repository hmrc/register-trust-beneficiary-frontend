...
class DateOfBirthYesNoController @Inject()(
  ...
  private val form: Form[Boolean] = yesNoFormProvider.withPrefix("individualBeneficiaryDateOfBirthYesNo")
  ...
  def onSubmit(index: Int, draftId: String): Action[AnyContent] = actions(index, draftId).async {
    implicit request =>
      ...
      form.bindFromRequest().fold(
        ...
        value => {
          val result = for {
            updatedAnswers <- EitherT(Future.successful(request.userAnswers.set(DateOfBirthYesNoPage(index), value)))
