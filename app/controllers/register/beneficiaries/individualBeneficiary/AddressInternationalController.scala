...
class AddressInternationalController @Inject()(
  ...
  def onPageLoad(index: Int, draftId: String): Action[AnyContent] = actions(index, draftId) {
    implicit request =>
      ...
      Ok(view(preparedForm, countryOptions.options(request.lang(messagesApi)), index, draftId, name.toString))
  }
    ...
    implicit request =>
      ...
      form.bindFromRequest().fold(
        (formWithErrors: Form[_]) =>
          Future.successful(BadRequest(view(formWithErrors, countryOptions.options(request.lang(messagesApi)), index, draftId, name.toString))),
