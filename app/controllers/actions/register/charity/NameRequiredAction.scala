...
package controllers.actions.register.charity
...
class NameRequiredActionAction(index: Int)(implicit val executionContext: ExecutionContext, val messagesApi: MessagesApi)
  ...
  private def getName[A](request: RegistrationDataRequest[A]): String = {
    request.userAnswers.get(CharityNamePage(index)) match {
      case Some(name) => name
      case _ => request.messages(messagesApi)("charityBeneficiary.name.default")
    }
  }
}
