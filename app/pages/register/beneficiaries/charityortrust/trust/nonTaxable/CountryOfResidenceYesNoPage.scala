...
final case class CountryOfResidenceYesNoPage(index : Int) extends QuestionPage[Boolean] {
  ...
  override def cleanup(value: Option[Boolean], userAnswers: UserAnswers): Try[UserAnswers] =
    value match {
      case Some(false) => userAnswers.remove(AddressUKYesNoPage(index))
        .flatMap(_.remove(AddressUKPage(index))) //TODO remove CountryOfResidenceAddressUKPage
        .flatMap(_.remove(AddressInternationalPage(index))) //TODO remove CountryOfResidenceInternationalAddressPage
