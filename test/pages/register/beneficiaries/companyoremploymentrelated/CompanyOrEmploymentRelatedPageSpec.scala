        ...
        "last of other beneficiary types are in progress" in {
          forAll(arbitrary[UserAnswers]) {
            initial =>
              ...
              val result = answers.set(CompanyOrEmploymentRelatedPage, Company).success.value
        ...
        "last of other beneficiary types are in progress" in {
          forAll(arbitrary[UserAnswers]) {
            initial =>
              ...
              val result = answers.set(CompanyOrEmploymentRelatedPage, EmploymentRelated).success.value
      ...
      def runTestsForType(beneficiaryType: CompanyOrEmploymentRelatedToAdd): Unit = {
        ...
        s"$beneficiaryType selected" when {
          ...
          "no other beneficiary types" in {
            forAll(arbitrary[UserAnswers]) {
              initial =>
                val result = initial.set(CompanyOrEmploymentRelatedPage, beneficiaryType).success.value
          ...
          "last of other beneficiary types is complete" in {
            forAll(arbitrary[UserAnswers]) {
              initial =>
                ...
                val result = answers.set(CompanyOrEmploymentRelatedPage, beneficiaryType).success.value
