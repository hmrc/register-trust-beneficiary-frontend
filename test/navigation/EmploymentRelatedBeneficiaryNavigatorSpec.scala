  ...
  "Employment related beneficiary navigator" must {
    ...
    "go to AddressUkYesNo from AddressYesNo if Yes" in
      forAll(arbitrary[UserAnswers]) { baseAnswers =>
        val answers = baseAnswers.set(LargeBeneficiaryAddressYesNoPage(index), true).value
        navigator
          .nextPage(LargeBeneficiaryAddressYesNoPage(index), fakeDraftId, answers)
          .mustBe(rts.AddressUkYesNoController.onPageLoad(index, fakeDraftId))
      }

    "go to Description from AddressYesNo if No" in
      forAll(arbitrary[UserAnswers]) { baseAnswers =>
        val answers = baseAnswers.set(LargeBeneficiaryAddressYesNoPage(index), false).value
        navigator
          .nextPage(LargeBeneficiaryAddressYesNoPage(index), fakeDraftId, answers)
    ...
    "go to UkAddress from AddressYesUkNo if Yes" in
      forAll(arbitrary[UserAnswers]) { baseAnswers =>
        ...
        navigator
          .nextPage(LargeBeneficiaryAddressUKYesNoPage(index), fakeDraftId, answers)
          .mustBe(rts.UkAddressController.onPageLoad(index, fakeDraftId))
      }

    "go to NonUkAddress from AddressYesUkNo if No" in
      forAll(arbitrary[UserAnswers]) { baseAnswers =>
        val answers = baseAnswers.set(LargeBeneficiaryAddressUKYesNoPage(index), false).value
        navigator
          .nextPage(LargeBeneficiaryAddressUKYesNoPage(index), fakeDraftId, answers)
    ...
    "go to Description from UkAddress " in
      forAll(arbitrary[UserAnswers]) { userAnswers =>
        navigator
          .nextPage(LargeBeneficiaryAddressPage(index), fakeDraftId, userAnswers)
          .mustBe(rts.DescriptionController.onPageLoad(index, fakeDraftId))
      }
    ...
    "go to Description from NonUkAddress " in
      forAll(arbitrary[UserAnswers]) { userAnswers =>
        navigator
          .nextPage(LargeBeneficiaryAddressInternationalPage(index), fakeDraftId, userAnswers)
          .mustBe(rts.DescriptionController.onPageLoad(index, fakeDraftId))
      }
    ...
    "go to NumberOfBeneficiaries from Description" in
      forAll(arbitrary[UserAnswers]) { userAnswers =>
        navigator
          .nextPage(LargeBeneficiaryDescriptionPage(index), fakeDraftId, userAnswers)
    ...
    "go to CheckAnswers from NumberOfBeneficiaries" in
      forAll(arbitrary[UserAnswers]) { userAnswers =>
        navigator
          .nextPage(LargeBeneficiaryNumberOfBeneficiariesPage(index), fakeDraftId, userAnswers)
    ...
    "a taxable trust" must {
      ...
      "Name page -> CountryOfResidence Yes No page" in
        navigator
          .nextPage(LargeBeneficiaryNamePage(index), draftId, baseAnswers)
