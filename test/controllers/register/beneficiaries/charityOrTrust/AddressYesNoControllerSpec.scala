...
class AddressYesNoControllerSpec extends SpecBase with MockitoSugar {
  ...
  val form = formProvider.withPrefix("addressYesNo")
  val index: Int = 0
  val charityName = "Test"

  lazy val addressYesNo = controllers.register.charityOrTrust.routes.AddressYesNoController.onPageLoad(NormalMode, index, draftId).url
