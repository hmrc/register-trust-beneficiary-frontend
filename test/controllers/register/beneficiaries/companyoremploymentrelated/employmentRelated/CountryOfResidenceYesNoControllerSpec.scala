...
import pages.register.beneficiaries.companyoremploymentrelated.employmentRelated.{CountryOfResidenceYesNoPage, LargeBeneficiaryNamePage}
import play.api.data.Form
import play.api.inject.bind
import play.api.test.FakeRequest
import play.api.test.Helpers._
import views.html.register.beneficiaries.companyoremploymentrelated.employmentRelated.CountryOfResidenceYesNoView

class CountryOfResidenceYesNoControllerSpec extends SpecBase with MockitoSugar {

  val formProvider = new YesNoFormProvider()
  val form: Form[Boolean] = formProvider.withPrefix("employmentRelatedBeneficiary.countryOfResidenceYesNo")
  val index: Int = 0
  val trustName = "Test"
  ...
  "CountryOfResidenceYesNo Controller" must {
    ...
    "return OK and the correct view for a GET" in {
      ...
      val userAnswers = emptyUserAnswers
        .set(LargeBeneficiaryNamePage(index), trustName).success.value
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = emptyUserAnswers.set(LargeBeneficiaryNamePage(index), trustName).success.value
    ...
    "redirect to the next page when valid data is submitted" in {
      ...
      val application = applicationBuilder(userAnswers = Some(userAnswers))
        .overrides(
          bind[Navigator].qualifiedWith(classOf[EmploymentRelatedBeneficiary]).toInstance(new FakeNavigator)
        ).build()
